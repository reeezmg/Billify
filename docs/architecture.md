...
- `src/pages/Tenants.tsx`: create, edit, and list tenants in natural ascending room-number order; hide inactive rows by default with a Show deleted toggle; open tenant bill history; export active tenants to a four-column meter-reading workbook (`Rm No`, `Name`, `Old`, `New`); download a formatted Excel import template; validate uploaded `.xlsx` rows; and bulk-create valid tenants while reporting row-level errors
